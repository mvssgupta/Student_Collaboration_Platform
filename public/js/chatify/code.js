  ...
  if (messenger != 0) {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        $(".messenger-infoView")
          .find(".avatar")
          .css("background-image", 'url("' + data.fetch.profilepic + '")');
        $(".header-avatar").css(
          "background-image",
          'url("' + data.fetch.profilepic + '")'
        );
      ...
      $.ajax({
        ...
        success: (data) => {
          ...
          if (contactsPage < 2) {
            ...
            $(".listOfGroups").append(data.contacts);
          }
          updateSelectedContact();
          // update data-action required with [responsive design]
          cssMediaQueries();
...
$(document).ready(function () {
  ...
  $("body").on("click", ".messenger-list-item-group", function () {
    const dataView = $(".messenger-list-item-group")
      .find("p[data-type]")
      .attr("data-type");
    $(".messenger-tab").hide();
    $(".messenger-tab[data-view=" + dataView + "s]").show();

    $(".messenger-list-item-group").removeClass("m-list-active");
    $(this).addClass("m-list-active");
    const userID = $(this).attr("data-contact");
    routerPush(document.title, `${url}/group/${userID}`);
    updateSelectedContact(userID);
  });
  ...
  $(".app-modal[data-name=delete]")
    ...
    .on("click", function () {
     ...
     // scp_logout modal [cancel button]
  $(".app-modal[data-name=scplogout]")
  .find(".app-modal-footer .cancel")
  .on("click", function () {
    app_modal({
      show: false,
   ...
   $("body").on("click", ".scplogout", function (e) {
    e.preventDefault();
    app_modal({
      show: true,
      name: "scplogout",
