...
{{-- -------------------- Group Item -------------------- --}}
@if($get == 'group')
<table class="messenger-list-item-group" data-contact="{{ $user->id }}">
    <tr data-action="0">
        {{-- Avatar side --}}
        <td style="position: relative">
            {{-- @if($user->active_status)
            ...
            @endif --}}
        <div class="avatar av-m"
        style="background-image: url('{{ asset('/storage/'.config('chatify.user_avatar.folder').'/'.$user->id) }}');">
        </div>
        </td>
        {{-- center side --}}
        <td>
        <p data-id="{{ $user->id }}" data-type="group">
            {{ strlen($user->groupname) > 12 ? trim(substr($user->groupname,0,12)).'..' : $user->groupname }}
            {{-- <span>{{ $lastMessage->created_at->diffForHumans() }}</span></p> --}}
        </td>
